...
If you install either the `vagrant-hostmanager` (installed by default unless removed from `vagrant_plugins` in your `config.yml`) or `vagrant-hostsupdater` plugin, you might get a permissions error when Vagrant tries changing the hosts file. On a macOS or Linux workstation, you're prompted for a sudo password so the change can be made, but on Windows, you have to do one of the following to make sure the plugin works correctly:
